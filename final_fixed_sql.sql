-- Final fixed SQL script for interview scheduling
-- This fixes both GROUP BY and ORDER BY errors
...
-- Step 3: Verify the fix (FIXED - added created_at to GROUP BY)
SELECT 
  p.user_id,
  p.full_name,
  p.worker_status,
...
LEFT JOIN worker_documents wd ON p.user_id = wd.worker_id
WHERE p.role = 'worker'
GROUP BY p.user_id, p.full_name, p.worker_status, p.status, p.created_at
