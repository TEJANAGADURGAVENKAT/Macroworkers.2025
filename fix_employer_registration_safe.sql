-- Safe fix for employer registration issues (handles existing policies)
-- Run this in your Supabase SQL editor

BEGIN;

-- 1. Fix RLS policies for profiles table (drop existing ones first)
...
-- 5. Create employer-documents storage bucket (only if it doesn't exist)
...
-- 6. Drop existing storage policies first (to avoid conflicts)
...
-- 7. Create new RLS policies for storage.objects
