-- Fix syntax error in validation function
-- This migration fixes the CURRENT_TIME syntax error

BEGIN;

-- Drop any existing problematic functions
DROP FUNCTION IF EXISTS validate_worker_selection() CASCADE;
DROP FUNCTION IF EXISTS validate_task_assignment(UUID, UUID) CASCADE;
...
-- Create a simple validation function with correct syntax
...
BEGIN
  ...
  IF task_record.assignment_start_time IS NOT NULL AND task_record.assignment_end_time IS NOT NULL THEN
    ...
    IF current_time_val < task_record.assignment_start_time OR current_time_val > task_record.assignment_end_time THEN
      RETURN FALSE;
    END IF;
  END IF;
