-- FIX DEPENDENCY ERROR - DROP ALL CONFLICTING TRIGGERS AND FUNCTIONS WITH CASCADE
-- This script uses CASCADE to remove all dependent objects and create a clean universal trigger

-- Step 1: Check what triggers and functions currently exist
SELECT 
    trigger_name,
    event_manipulation,
    action_timing,
    'EXISTING TRIGGER' as status
...
-- Step 2: Drop ALL conflicting functions with CASCADE (removes dependent triggers automatically)
DROP FUNCTION IF EXISTS check_assignment_limit_unified() CASCADE;
DROP FUNCTION IF EXISTS check_assignment_limit() CASCADE;
DROP FUNCTION IF EXISTS update_task_slot_count() CASCADE;
DROP FUNCTION IF EXISTS update_task_slot_counts_on_assignment() CASCADE;
DROP FUNCTION IF EXISTS update_task_assigned_count_universal() CASCADE;
DROP FUNCTION IF EXISTS update_task_assigned_count() CASCADE;

-- Step 3: Drop any remaining triggers manually (in case CASCADE didn't catch them)
...
-- Step 5: Create a CLEAN universal trigger function (no assignment limits)
...
-- Step 6: Create the CLEAN universal trigger (no assignment limits)
...
-- Step 7: Fix existing data - ensure all tasks have correct assigned_count
UPDATE tasks 
SET assigned_count = (
    SELECT COUNT(*) 
...
-- Step 8: Test with EXISTING tasks (should work now without assignment limits)
DO $$
DECLARE
...
-- Step 9: Test with NEWLY CREATED task (should work now)
DO $$
DECLARE
    new_task_id UUID;
    test_worker_id UUID;
...
-- Step 10: Show current status of ALL tasks
SELECT 
...
-- Step 11: Show individual task status
SELECT 
    id,
    title,
    max_workers,
...
-- Step 12: Verify ONLY our clean trigger is active
SELECT 
    trigger_name,
    event_manipulation,
    action_timing,
